...
/**
 * Model Context Protocol glyph. Authored as a fill-based icon (boxicons), so it
 * uses `fill="currentColor"` and inherits its color from the surrounding text —
 * sizing is driven by `className` (e.g. `h-3 w-3`) like the lucide icons it sits
 * alongside in the chat tool rows.
 */
export function McpIcon({ className, ...props }: SVGProps<SVGSVGElement>) {
  return (
    <svg

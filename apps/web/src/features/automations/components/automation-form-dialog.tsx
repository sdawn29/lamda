...
export function AutomationFormDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        {/* Keyed on open + the record being edited: forces a fresh mount (and
            therefore fresh form state initialized from props) every time the
            dialog opens, instead of syncing an already-mounted form via an
            effect. */}
        <AutomationFormBody
          key={`${open}:${automation?.id ?? "new"}`}
...
function AutomationFormBody({
  ...
  const [preset, setPreset] = useState<SchedulePreset>(
    () => parseCron(automation?.cron ?? "0 9 * * *").preset
  )
  const [minute, setMinute] = useState(
    () => parseCron(automation?.cron ?? "0 9 * * *").minute
  )
  const [hour, setHour] = useState(
    () => parseCron(automation?.cron ?? "0 9 * * *").hour
  )
  const [weekday, setWeekday] = useState(
    () => parseCron(automation?.cron ?? "0 9 * * *").weekday
  )
  const [custom, setCustom] = useState(
    () => parseCron(automation?.cron ?? "0 9 * * *").custom
  )
  const [modelId, setModelId] = useState<string | null>(
    () => automation?.modelId ?? null
  )
  const [mode, setMode] = useState(() => automation?.mode ?? "agent")
  const [approvalMode, setApprovalMode] = useState<AutomationApprovalMode>(
    () => automation?.approvalMode ?? "all_allowed"
  )
  const [useWorktree, setUseWorktree] = useState(
    () => automation?.useWorktree ?? true
  )

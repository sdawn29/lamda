...
function ThreadTabIcon({ threadId }: { threadId: string }) {
  ...
  if (status === "streaming")
    return <Loader2 className="size-3.5 shrink-0 animate-spin opacity-60" />
  if (status === "completed")
    return <span className="size-1.5 shrink-0 rounded-full bg-green-500" />
  if (status === "error")
...
export function MainTabBar() {
  ...
  const handleCloseTab = (e: React.MouseEvent, tab: MainTab) => {
    ...
    if (tab.type === "thread" && pendingThreadIds.has(tab.threadId)) {
      const ws = workspaces.find((w) =>
        w.threads.some((t) => t.id === tab.threadId)
      )
      if (ws) {
  ...
  return (
    <>
      <div className="flex h-9 shrink-0 scrollbar-none items-center gap-0.5 overflow-x-auto border-b bg-background px-1">
        {tabs.map((tab) => {
          const isActive = tab.id === activeTabId
          ...
          return (
            <div
              ...
              className={cn(
                "group relative flex h-7 shrink-0 cursor-pointer items-center gap-1.5 rounded-md pr-1.5 pl-2.5 text-xs transition-all duration-150 select-none",
                isActive

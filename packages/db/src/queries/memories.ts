...
function visibleTo(workspaceId?: string) {
  const scopeMatch = workspaceId
    ? or(
        ...
        and(
          eq(agentMemories.scope, "workspace"),
...
export function insertMemory(params: {
  ...
  db.insert(agentMemories)
    .values({
      ...
      filePaths:
        params.filePaths && params.filePaths.length
...
export function updateMemory(
  ...
  if (filePaths !== undefined) {
    set.filePaths =
      filePaths && filePaths.length ? JSON.stringify(filePaths) : null;
  }
...
export function upsertMemoryVector(id: string, embedding: number[]): void {
  ...
  try {
    ...
    db.run(
      sql`INSERT INTO agent_memories_vec(id, embedding) VALUES (${id}, ${json})`,
    );
  } catch {
 ...
 */
export function findMemoryByTitle(
  title: string,
  workspaceId?: string,
): MemoryRow | undefined {
  return db
    ...
    .where(
      and(
        ...
        sql`lower(${agentMemories.title}) = ${title.toLowerCase()}`,
      ),
    )
...
export function listMemories(filter?: {
  ...
  if (filter?.workspaceId)
    conditions.push(eq(agentMemories.workspaceId, filter.workspaceId));
  return db
    .select()
    .from(agentMemories)
  ...
  return db
    ...
    .where(
      and(
        visibleTo(workspaceId),
        or(
          like(agentMemories.title, pattern),
          like(agentMemories.content, pattern),
        ),
      ),
    )
    .orderBy(desc(agentMemories.updatedAt))
...
export function retrieveByFilePaths(
  ...
  const rows = db
    ...
    .from(agentMemories)
    .where(
      and(visibleTo(workspaceId), sql`${agentMemories.filePaths} IS NOT NULL`),
    )
    .orderBy(desc(agentMemories.updatedAt))
    .all() as MemoryRow[];
  const out: MemoryRow[] = [];
  for (const row of rows) {
    ...
    if (
      ...
      parsed.some((p) => typeof p === "string" && wanted.has(p))
    ) {
      out.push(row);
      if (out.length >= limit) break;
    }
...
export function selectMemoriesForPrompt(
  ...
  const byFile = activeFiles?.length
    ? retrieveByFilePaths(activeFiles, workspaceId)
    : [];

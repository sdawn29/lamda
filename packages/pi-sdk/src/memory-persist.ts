...
export async function persistMemory(
  ...
  if (looksLikeSecret(title) || looksLikeSecret(content)) {
    return {
      outcome: "rejected",
      message:
        "Refused: the title or content looks like a secret (key/token/password).",
    };
  }
  ...
  const embedding =
    (await embedDocuments([`${title}\n${content}`]).catch(() => null))?.[0] ??
    null;
  ...
  if (embedding) {
    ...
    if (nearest && nearest.distance <= DEDUP_DISTANCE) {
      ...
      if (input.filePaths?.length) {
        let existing: string[] = [];
        try {
          const parsed = nearest.row.filePaths
            ? JSON.parse(nearest.row.filePaths)
            : [];
        ...
        if (merged.length !== existing.length)
          updateMemory(nearest.row.id, { filePaths: merged });
      }
